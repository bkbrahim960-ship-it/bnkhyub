...
function isMongoConnected() {
  return typeof mongoose.isConnected === 'function' && mongoose.isConnected();
}
...
async function getTeamsMap() {
    ...
    let teams;
    if (isMongoConnected()) {
      teams = await Team.find({}, 'id name_en name_fa').lean();
    } else {
      teams = jsonDb.find('teams', {});
    }
...
router.get('/groups', async(req,res) => {
    try{
        ...
        if (isMongoConnected()) {
            ...
            groups = jsonDb.find('matchtables', {});
        }
...
router.get('/group', async(req,res) => {
    try{
        ...
        let group;
        if (isMongoConnected()) {
            group = await Group.findOne({name: req.query.name});
            const teams = await Team.find({group: group._id});
            return res.send({group, teams});
        } else {
            group = jsonDb.findOne('matchtables', {name: req.query.name.toUpperCase()});
            const teams = jsonDb.find('teams', {groups: req.query.name.toUpperCase()});
            return res.send({group, teams});
        }
    }catch(err){
        return res.status(400).send({
...
router.get('/team/:idTeam', async(req,res) => {
    try{
        let team;
        if (isMongoConnected()) {
            team = await Team.findById(req.params.idTeam).lean();
        } else {
            team = jsonDb.findOne('teams', {id: req.params.idTeam});
        }
...
router.get('/team', async(req,res) => {
    try{
        ...
        let team;
        if (isMongoConnected()) {
            team = await Team.findOne({name_en: name});
        } else {
            team = jsonDb.findOne('teams', {name_en: name});
        }
        return res.send({team});
    }catch(err){
        return res.status(400).send({
...
router.get('/teams', async(req,res) => {
    try{
        ...
        if (isMongoConnected()) {
            ...
            const filter = req.query.group ? {groups: req.query.group.toUpperCase()} : {};
            teams = jsonDb.find('teams', filter);
        }
...
router.get('/game/:idGame', async(req,res) => {
    try{
        let game;
        if (isMongoConnected()) {
            game = await Game.findById(req.params.idGame).lean();
        } else {
            game = jsonDb.findOne('matches', {id: req.params.idGame});
        }
...
router.get('/stadiums', async(req,res) => {
    try{
        let stadiums;
        if (isMongoConnected()) {
            stadiums = await Stadium.find().lean();
        } else {
            stadiums = jsonDb.find('stadiums', {});
        }
...
router.get('/stadium/:id', async(req,res) => {
    try{
        let stadium;
        if (isMongoConnected()) {
            stadium = await Stadium.findOne({id: req.params.id});
        } else {
            stadium = jsonDb.findOne('stadiums', {id: req.params.id});
        }
